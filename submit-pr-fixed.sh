#!/bin/bash
# Submit Performance Optimization PR - Fixed Version
# Usage: GITHUB_TOKEN=xxx ./submit-pr-fixed.sh
...
if [ -z "$TOKEN" ]; then
  ...
  echo "Usage: GITHUB_TOKEN=xxx ./submit-pr-fixed.sh"
  exit 1
fi
...
BODY=$(cat /tmp/pr_body.json | jq -Rs .)
...
RESPONSE=$(curl -s -X POST \
  ...
  --data-raw "{\"title\":\"${TITLE}\",\"body\":${BODY},\"head\":\"${BRANCH}\",\"base\":\"main\"}")
